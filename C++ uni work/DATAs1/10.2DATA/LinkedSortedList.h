...
public:
	...
	bool removeK(const ItemType& anEntry);
	void clear();
...
template<class ItemType>
bool LinkedSortedList<ItemType>::removeK(const ItemType& anEntry)
{
...
class Client {
public:
	char statusOfClient;  // 'A' for arrival, 'D' for departure
	int transactionTime;
	int arrivalTime;

	Client(char status, int transTime, int arrTime)
		: statusOfClient(status), transactionTime(transTime), arrivalTime(arrTime) {}
