...
protected:
    ...
    BinaryNode<ItemType>* moveValuesUpTree(BinaryNode<ItemType>* subTreePtr, bool& success);
...
ItemType BinaryNodeTree<ItemType>::getRootData() const {
    if (isEmpty())
        throw std::runtime_error("TreeException: Empty tree");
...
ItemType BinaryNodeTree<ItemType>::getEntry(const ItemType& anEntry) const {
    BinaryNode<ItemType>* result = nullptr;
    bool success = false;
    result = find(rootPtr, anEntry, success);
    if (!success)
        throw std::runtime_error("TreeException: Entry not found");
...
BinaryNode<ItemType>* BinaryNodeTree<ItemType>::removeValue(BinaryNode<ItemType>* subTreePtr, const ItemType target, bool& success) {
    ...
    if (subTreePtr->getItem() == target) {
        subTreePtr = moveValuesUpTree(subTreePtr, success);
        success = true;
    }
...
template <class ItemType>
BinaryNode<ItemType>* BinaryNodeTree<ItemType>::moveValuesUpTree(BinaryNode<ItemType>* subTreePtr, bool& success) {
    BinaryNode<ItemType>* leftPtr = subTreePtr->getLeftChildPtr();
    BinaryNode<ItemType>* rightPtr = subTreePtr->getRightChildPtr();
    ...
    if (leftPtr == nullptr && rightPtr == nullptr) {
        ...
        BinaryNode<ItemType>* nodeToConnectPtr = rightPtr;
        delete subTreePtr;
        return nodeToConnectPtr;
    }
    else if (rightPtr == nullptr) {
        BinaryNode<ItemType>* nodeToConnectPtr = leftPtr;
        delete subTreePtr;
        return nodeToConnectPtr;
    }
    else {
        BinaryNode<ItemType>* tempPtr = rightPtr;
        while (tempPtr->getLeftChildPtr() != nullptr)
            tempPtr = tempPtr->getLeftChildPtr();
        subTreePtr->setItem(tempPtr->getItem());
        bool success;
        subTreePtr->setRightChildPtr(removeValue(rightPtr, tempPtr->getItem(), success));
        return subTreePtr;
    }
}
